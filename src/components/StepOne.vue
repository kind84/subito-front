...
export default {
  ...
  watch: {
    attList() {
      if (this.attList.length > 0 && this.attList.filter(a => {return a !== ""}).length === this.attNo) {
        this.complete = true
    ...
    attNo() {
      if (this.attList.length !== this.attNo) {
        this.complete = false
        this.$emit("complete1", false)
      }
      if (this.attList.filter(a => {return a !== ""}).length === this.attNo) {
        this.complete = true
        this.$emit("complete1", true)
      }
    }
